...
$sql_check = "SELECT * FROM sub_kriteria WHERE id_sub_kriteria = '$id_sub_kriteria'";
$result_check = $conn->query($sql_check);

if ($result_check->num_rows == 0) {
  echo '<script>alert("Kriteria tidak ditemukan")</script>';
  echo '<script>window.location="../sub_lihat_kriteria.php"</script>';
  exit();
}
                        ...
                        <div class="col-lg-12">
                          <div class="card">
                            <div class="card-body">
                              ...
                              <form id="formKriteria" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id_sub_kriteria; ?>" method="post">
                                <div class="row mb-3">
                                ...
                                <div class="row mb-3">
                                  ...
                                  <div class="col-sm-10">
                                    <input type="text" class="form-control" name="sub_kriteria" id="sub_kriteria" value="<?php echo $data_kriteria['sub_kriteria']; ?>" required></input>
                                  </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                  ...
                                  <div class="col-sm-10">
                                    <input type="text" class="form-control" name="bobot_sub_kriteria" id="bobot_sub_kriteria" value="<?php echo $data_kriteria['bobot_sub_kriteria']; ?>" required></input>
                                  </div>
                                </div>

                                <button type="submit" class="btn btn-primary">
                                  <i class="bi bi-pencil-square"></i> Update Subkriteria
                                </button>
                              </form>
                            </div>
                          </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql_update = "UPDATE sub_kriteria SET kode_kriteria = '$kode_kriteria', sub_kriteria = '$sub_kriteria', bobot_sub_kriteria = '$bobot_sub_kriteria' WHERE id_sub_kriteria = '$id_sub_kriteria'";
  ...
  if ($conn->query($sql_update) === TRUE) {
    ...
    echo '<script>
            Swal.fire({
                icon: "success",
                title: "Subkriteria berhasil diperbarui!",
                text: "Selamat! Data subkriteria telah diperbarui.",
                confirmButtonColor: "#3085d6",  // Warna tombol "OK"
                confirmButtonText: "OK",
            }).then(function() {
                window.location.href = "sub_lihat_kriteria.php";
            });
        </script>';
    exit();
  } else {
