...
<head>
  ...
  <link href="assets/assets/img/terapan/logo-large.png" rel="icon">
...
<body>
                    ...
                    <div class="card-body">
                      ...
                      <form id="formKriteria" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <!-- <div class="row mb-3">
													...
													<div class="col-sm-10">
														<input type="number" class="form-control" name="bobot_kriteria" id="bobot_kriteria" required></input>
													</div>
												</div> -->

                        <!-- Hapus bagian untuk mengunggah gambar -->
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi apakah kode kriteria sudah ada dalam database
    $cek_duplicate = "SELECT COUNT(*) AS total FROM kriteria WHERE kode_kriteria = '$kode_kriteria'";
    $result = $conn->query($cek_duplicate);
    $row = $result->fetch_assoc();
    if ($row['total'] > 0) {
      // Jika kode kriteria sudah ada, berikan peringatan
      echo '<script>
            Swal.fire({
                icon: "error",
                title: "Kode Kriteria Sudah Ada",
                text: "Kode kriteria yang Anda masukkan sudah ada dalam database. Mohon gunakan kode yang berbeda.",
                confirmButtonColor: "#d33",
                confirmButtonText: "Tutup",
            });
            </script>';
    } else {
      // Jika kode kriteria belum ada, masukkan data ke dalam tabel
      $sql = "INSERT INTO kriteria (kode_kriteria, nama_kriteria, jenis_kriteria) VALUES ('$kode_kriteria', '$nama_kriteria', '$jenis_kriteria')";
      ...
      if ($conn->query($sql) === TRUE) {
        ...
        echo '<script>
                Swal.fire({
                    ...
                    confirmButtonText: "Lihat Kriteria",
